...
function dump(encodedFrame: any, direction: 'send' | 'receive', mode: 'before' | 'after', max = 16) {
  ...
  console.debug(performance.now().toFixed(2), mode, direction, bytes.trim(),
...
self.onmessage = (event: MessageEvent<PeerWorkerMessage>) => {
