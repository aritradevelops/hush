...
export class Caller {
  ...
  async init() {
    ...
    if (this.isMuted) this.userMedia.getAudioTracks().forEach(t => t.kind === 'audio' && (t.enabled = false))
    if (this.isVideoOff) this.userMedia.getAudioTracks().forEach(t => t.kind === 'audio' && (t.enabled = false))
  }
