...
export abstract class Repository<T extends typeof PrimaryColumns = typeof PrimaryColumns> {
  ...
  protected getListQuery(query: ListParams) {
    const qb = this.entity.createQueryBuilder(this.entity.name)
      // @ts-ignore
      .select(query.select ? query.select.split(',').map(field => this.getColumnName(field)) : query.select);
    if (query.search) {
      qb.where(`${this.getColumnName('search')} @@ to_tsquery(:search)`, { search: query.search.trim().split(" ").join(":* & ").concat(":*") });
    }
    ...
    if (query.trash) {
      qb.andWhere(`${this.getColumnName('deleted_at')} IS NOT NULL`);
    } else {
      qb.andWhere(`${this.getColumnName('deleted_at')} IS NULL`);
    }
    return qb
  }
